...
public class FillFluidContainerInteraction extends SimpleInstantInteraction {
    ...
    protected void firstRun(InteractionType interactionType, InteractionContext interactionContext, CooldownHandler cooldownHandler) {
        ...
        try{
            ...
            playerRef.sendMessage(Message.raw("You have used the custom item: " + itemStack.getItemId() + " ("+durability+"/"+maxDurability+")"));
            playerRef.sendMessage(Message.raw("Your target block is: "+blockType+"  (" + targetPos.x +","+ targetPos.y+","+ targetPos.z + ")"));
            playerRef.sendMessage(Message.raw("above your target block is: "+aboveBlockType+"  (" + targetPos.x +","+ (targetPos.y+1)+","+ targetPos.z + ")"));
        ...
        try {
            ...
            if(aboveTargetFluid!=7){ playerRef.sendMessage(Message.raw(aboveTargetFluid+" != "+7)); }
            if(aboveTargetFluid==7 && a.equals(b)){
                playerRef.sendMessage(Message.raw("Found water!"));
